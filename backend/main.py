...
from api.crafteo import crafteo_bp
...
app.register_blueprint(crafteo_bp, url_prefix="/api/crafteo")
...
def obtener_slots():
    ...
    for slot_num in range(1, SaveManager.NUM_SLOTS + 1):
        ...
        slots.append({"numero": slot_num, "ocupado": info is not None, "info": info})
...
def obtener_slot(slot_num):
    ...
    info = save_manager.obtener_info_slot(slot_num)
    return jsonify({"numero": slot_num, "ocupado": info is not None, "info": info})
...
def nueva_partida():
    ...
    if dificultad not in dificultades_validas:
        return api_error(
            f"Dificultad inválida. Debe ser una de: {dificultades_validas}"
        )
    ...
    personaje, mensaje_creacion = crear_nuevo_personaje(
        nombre=nombre, genero=genero, dificultad=dificultad
    )
...
def obtener_personaje(slot_num):
    ...
    return jsonify({"success": True, "data": datos.get("personaje", {})})
